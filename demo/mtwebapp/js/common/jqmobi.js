        ...
        */
        $.ajax = function(opts) {
            ...
            try {
                ...
                if ($.isObject(settings.data) && !(window.FormData && settings.data instanceof FormData))
                ...
                if (settings.contentType && !(window.FormData && settings.data instanceof FormData))
                ...
                if (settings.timeout > 0)
                    abortTimeout = setTimeout(function() {
                        ...
                        settings.complete.call(context, xhr, 'error'); // fixed by rocksun 超时后调用 complete
                    }, settings.timeout);
...
if(define){
	define('jqmobi',[],function () {console.log('jqmobi');return $});
}
